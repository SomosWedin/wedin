...
describe('event-domain helpers', () => {
  ...
  it.each(['abc', 'amelie-y-john', 'a1-b2', `a${'b'.repeat(61)}c`])(
  ...
  it('normalizes the configured root domain', () => {
    process.env.NEXT_PUBLIC_ROOT_DOMAIN = ' SOMOSWEDIN.COM.:443 '

    expect(getConfiguredRootDomain()).toBe('somoswedin.com')
  })
  ...
  it('extracts a localhost event slug while ignoring the port', () => {
    expect(
      getEventSlugFromHost('amelie-y-john.localhost:3000', 'localhost')
    ).toBe('amelie-y-john')
  })
  ...
  it.each([
    ...
    expect(getEventSlugFromHost(host, 'somoswedin.com')).toBeNull()
  })

  it('builds a production event URL with a clean public path', () => {
    expect(getPublicEventUrl('amelie-y-john', '/checkout')).toBe(
  ...
  it.each(['invalid.slug', '-invalid', 'invalid-', 'UPPERCASE'])(
    'refuses to build a public URL for the invalid slug %s',
    slug => {
  ...
  it('keeps internal /e routes out of browser-facing event paths', () => {
    ...
    expect(publicEventPaths.bankTransfer(['tx-1', 'tx-2'])).toBe(
